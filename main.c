...
void mapearDistancia(int matriz[][TAM_COLUNAS])
{
    ...
    while (1)
    {
        ...
        if (contador > 400)
        {
            break; // Sai do loop se não houver mais mudanças ou após um limite de iterações
        }
    }
}
...
int menorPonto(int posX, int posY, int matriz[][TAM_COLUNAS])
{
    ...
    // Calcula as distâncias a partir de posições vizinhas na matriz
    if (posX > 0)
    {
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            menor = pontos[i]; // Encontra a menor distância entre as posições vizinhas
        }
    }
    return menor;
}
